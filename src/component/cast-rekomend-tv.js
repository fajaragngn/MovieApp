...
class CastDanRekomenTV extends React.Component {
  ...
  render() {
    const cast =
      this.state.tvCast.length === 0
        ? "We don't have enough data Cast about this TV shows."
        : this.state.tvCast.map((cazt) => {
    ...
    const Recomend =
      this.state.tv.length === 0
        ? "We don't have enough data to suggest suggest about this TV shows."
        : this.state.tv.map((rec) => {
    ...
    const ReviewCards = this.state.review.slice(0, 1).map((rev) => {
      ...
      return (
        <ReviewCard img={img} data={rev} length={this.state.review.length} />
      );
    });
    ...
    return (
      <>
        ...
        <div className="review-container">
          ...
          <div className="review-content">
            {this.state.review.length < 1
              ? `We don't have any reviews for ${this.props.judul}.`
              : ReviewCards}
            <Link
              to={ID}
            ...
            <div className="tittle-inners">
              <ul>
                <li>
                  <a
                    href="#"
                    className={this.state.option === "Populer" ? "media" : ""}
                    data-id="Populer"
                ...
                <li>
                  <a
                    href="#"
                    className={this.state.option === "Trailer" ? "media" : ""}
                    data-id="Trailer"
                    onClick={this.mediaClick}
                  >
                ...
                <li>
                  <a
                    href="#"
                    className={this.state.option === "Backdrop" ? "media" : ""}
                    data-id="Backdrop"
                    onClick={this.mediaClick}
                  >
                ...
                <li>
                  <a
                    href="#"
                    className={this.state.option === "Poster" ? "media" : ""}
                    data-id="Poster"
                    onClick={this.mediaClick}
                  >
              ...
              {this.state.option === "Backdrop" ? (
                <div className="view-all">
                  <Link to={backID} className="view">
                    View All
                  </Link>
                </div>
              ) : (
              ...
              {this.state.option === "Poster" ? (
                <div className="view-all">
                  <Link to={posterID} className="view">
                    View All
                  </Link>
                </div>
              ) : (
        ...
        <div className="populer-container">
          ...
          <div className="detail-inner">{Recomend}</div>
        </div>
      </>
    );
  }
