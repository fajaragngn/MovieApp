...
class BackdropCards extends React.Component {
  ...
  render() {
    const ID = this.state.dataTv.type
      ? `/tv-detail/${this.props.id}`
      : `/movie-detail/${this.props.id}`;
    const date = this.state.dataTv.type
      ? new Date(this.state.dataTv.first_air_date)
      : new Date(this.state.dataMovie.release_date);
    console.log(this.state.dataMovie);
    return (
      <>
        <div className="poster-header">
          <div className="poster-inner">
            {this.state.dataTv.type ? (
              <Link to={ID}>
                <img
                  src={`https://image.tmdb.org/t/p/w300/${this.state.dataTv.poster_path}`}
                />
              </Link>
            ) : (
              <Link to={ID}>
                <img
                  src={`https://image.tmdb.org/t/p/w300/${this.state.dataMovie.poster_path}`}
                />
              </Link>
            )}
            <div className="poster-title">
              {this.state.dataTv.type ? (
                ...
                <>
                  ...
                  <h3>
                    <Link to={ID}>
                      {this.state.dataMovie.title} ({date.getFullYear()})
                    </Link>
                  </h3>
                  <Link to={ID}> Back</Link>
                </>
