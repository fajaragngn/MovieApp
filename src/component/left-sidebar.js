...
class LeftSidebar extends React.Component {
  ...
  async componentDidMount() {
    const genreRes = await fetch(
      "https://api.themoviedb.org/3/genre/movie/list?api_key=0ccbee0a69447c2b1bd0090bf76b0358"
    );
    const genreTv = await fetch(
      "https://api.themoviedb.org/3/genre/tv/list?api_key=0ccbee0a69447c2b1bd0090bf76b0358"
    );
  ...
  render() {
    ...
    const trending = this.state.trending.map((tren) => {
      ...
      return (
        <TrendingCard
          key={tren.id}
          index={this.state.indexTr}
          title={tren.name}
          img={tren.poster_path}
          id={tren.id}
          length={length++}
          rating={tren.vote_average}
        />
      );
    });
    ...
    return (
      <section className="left-sidebar">
        <div className="left-list">
          ...
          <div className="trending-option">
            <ul>
              <li>
                <a
                  href="#"
                  className={this.state.option === "day" ? "active" : ""}
                  data-id="day"
              ...
              <li>
                <a
                  href="#"
                  className={this.state.option === "week" ? "active" : ""}
                  data-id="week"
                  onClick={this.trendingClick}
                >
                  Minggu ini
          ...
          <div className="sidebar-content">{trending}</div>
        </div>
        <div className="left-list">
